...
def obs_to_text(obs):
    ...
    lidar_angles = [
        ...
        1.2,
        1.35,
    ]  # In radians starting from the top of the hull
    lidar_distances = obs[14:24]
    lidar_readings = [
        f"Lidar {i+1} ({angle:.2f} rad): {distance:.2f}"
        for i, (angle, distance) in enumerate(zip(lidar_angles, lidar_distances))
    ]
    ...
    return "\n".join(
        [
            ...
            x_velocity,
            y_velocity,
            back_revolute_joint_angle,
            back_revolute_joint_speed,
            back_lower_leg_angle,
            back_lower_leg_speed,
            back_leg_ground_contact_flag,
            front_revolute_joint_angle,
            front_revolute_joint_speed,
            front_lower_leg_angle,
            front_lower_leg_speed,
            front_leg_ground_contact_flag,
        ]
        + lidar_readings
    )
...
def leg_movement(
    ...
    no_check,
    threshold,
    previous_movements_back=[],
    previous_movements_front=[],
):
    """
    ...
    back_movement = {
        "hip_joint_1_angle": back_movement[0],
        "hip_joint_1_speed": back_movement[1],
        "lower_leg_1_angle": back_movement[2],
        "lower_leg_1_speed": back_movement[3],
    }
    front_movement = observations[9:13]
    front_movement = {
        "hip_joint_2_angle": front_movement[0],
        "hip_joint_2_speed": front_movement[1],
        "lower_leg_2_angle": front_movement[2],
        "lower_leg_2_speed": front_movement[3],
    }
    ...
    back_movements_dicts = previous_movements_back[-no_check:]
    front_movements_dicts = previous_movements_front[-no_check:]

    # Check to see if the back leg is moving (Not move if the value is very close to 0)

    back_movement_speed = sum(
        [
            (
                back_movements_dict["hip_joint_1_speed"]
                + back_movements_dict["lower_leg_1_speed"]
            )
            / 2
            for back_movements_dict in back_movements_dicts
        ]
    )
    front_movement_speed = sum(
        [
            (
                front_movements_dict["hip_joint_2_speed"]
                + front_movements_dict["lower_leg_2_speed"]
            )
            / 2
            for front_movements_dict in front_movements_dicts
        ]
    )

    # TODO: Could do something with the hip/joint angles as well

    back_res = (
        f"Back Leg has not moved the past {no_check} steps please try a different action to move the back leg"
        if not back_movement_speed
        else None
    )
    front_res = (
        f"Front Leg has not moved the past {no_check} steps please try a different action to move the front leg"
        if not front_movement_speed
        else None
    )
